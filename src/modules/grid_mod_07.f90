module grid_mod
   ! Builds frequency-dependent GQG meshes, maps acquisition points and
   ! material properties, and transfers models between grid representations.
...
!======================================================================
   SUBROUTINE BuildMeshForBand_FromCanon( &
     ...
     WRITE (*, '(A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') 'Original XMIN=', XMIN, 'XMAX=', XMAX, 'ZMIN=', ZMIN, 'ZMAX=', ZMAX
      ...
      IF (my_rank == 0 .AND. dbg) WRITE (*, '(A,F6.2)') 'calling ShiftCoordinates for FREQ ', FREQ
      ...
      IF (my_rank == 0 .AND. dbg) WRITE (*, '(A,F6.2)') 'calling GQG_2D for FREQ ', FREQ
      ...
      IF (my_rank == 0 .AND. dbg) THEN
         WRITE (*, *) '--- Shifted-coordinate and source-mapping check ---'
      ...
      IF (INV <= 1) THEN
         IF (.NOT. have_true_model) THEN
            ...
            END IF
            CALL fail('BuildMeshForBand_FromCanon: missing true-model arrays for INV=0 or INV=1')
         END IF
   ...
   !
   !     Inputs:
   !       WIDTH............. Multiplier applied to the maximum wavelength
   ...
   SUBROUTINE absorb_zone(WIDTH, WLMAX, XMIN, XMAX, ZMIN, ZMAX, DX, DZ, IS0, NORD, &
      ...
      Z0 = IE0*DZ0
      X0 = IE0*DX
      XMIN = XMIN - X0        ! Extend range for the GSRM boundary zone
      XMAX = XMAX + X0
   ...
   SUBROUTINE GQG_2D(NTO, XTO, ZTO, XMIN, XMAX, ZMIN, ZMAX, DX, DZ, NORD, &
      ...
      !       IE0............... Number of GSRM extension cells
      !       NSR............... Number of sources
      ...
      !       IG................ InversionGridType (geometry, GSRM mask, mappings)
      !
      !-----------------------------------------------------------------------
      IMPLICIT NONE
      ...
      IF (SIZE(X) < NX .OR. SIZE(XP) < NNX .OR. SIZE(ZP) < NPT) THEN
         ...
         END IF
         CALL fail('GQG_2D: mesh arrays not allocated to shifted sizes')
      END IF
      ...
      ! === Build inversion-grid geometry and GSRM mask from this mesh ===
      ...
      IF (my_rank == 0) THEN
         ...
         unit_grid = -1
         OPEN (newunit=unit_grid, FILE='grid.dat', STATUS='REPLACE', ACTION='WRITE')
...
29             FORMAT(I8, 2X, 2(F12.4, 4X))
      ...
      IF (SIZE(MSR1, 2) < max_nb .OR. SIZE(FSR, 2) < max_nb) THEN
         ...
         END IF
         CALL fail('GQG_2D: MSR1/FSR arrays too small for RD')
      END IF
      ...
      DO I = 1, NSR
         ...
         DO K = 1, NNX
            ...
            DO L = 1, NNZ
               ...
               IF (IE >= max_nb) THEN
                  ...
                  END IF
                  CALL fail('GQG_2D: IE exceeds RD/MSR1/FSR bounds')
               END IF
         ...
         IF (MSR(I) <= 0) THEN
            ...
            END IF
            CALL fail('GQG_2D: source with no GQG neighbours')
         END IF
         ...
         IF (MAXVAL(MSR1(I, 1:MSR(I))) > NPT_local .OR. &
            ...
            END IF
            CALL fail('GQG_2D: MSR1 index outside [1, NNX*NNZ]')
         END IF
      END DO
      ...
      DO I = 1, NSR
         ...
         ELSE
            ...
            IF (S <= 0.0_dp) THEN
               ...
               END IF
               CALL fail('GQG_2D: weight sum S <= 0')
            END IF
            DO J = 1, MSR(I)
               FSR(I, J) = FSR(I, J)/S
            END DO
...
25       FORMAT(I12, 3X, 4(F12.3, 3X))
   ...
   SUBROUTINE QC(IANISO, ITHOM, IVISCO, PARAM, NPAR, MX, XM, MZ, ZM, NNX, NNZ, NPT, XP, &
      ...
      !---- GSRM extension limits in x and z ---
      I1 = (NORD - 1)*IE0 + 1
      I2 = NNX - I1 + 1
      J1 = (NORD - 1)*IE0 + 1
      J2 = NNZ - J1 + 1
...
14                            RETURN
                              ...
                              SUBROUTINE npt2block_avg_single(NPT, NBLOCK, NORD, NNZ, N0_BLOCK, v_fine, v_block)
                                 ...
                                 IF (SIZE(v_fine) < NPT) CALL fail('npt2block_avg_single: v_fine too small')
                                 IF (SIZE(v_block) < NBLOCK) CALL fail('npt2block_avg_single: v_block too small')
                              ...
                              SUBROUTINE block2npt_const_single(NPT, NBLOCK, NORD, NNZ, N0_BLOCK, v_block, v_fine)
                                 ...
                                 IF (SIZE(v_block) < NBLOCK) CALL fail('block2npt_const_single: v_block too small')
                                 IF (SIZE(v_fine) < NPT) CALL fail('block2npt_const_single: v_fine too small')
...
!======================================================================
                              SUBROUTINE Project_CRR0_2_CR0( &
                                 ...
                                 IF (SIZE(CR0, 1) /= IANISO) THEN
                                    ...
                                    CALL fail('Project_CRR0_2_CR0: inconsistent CR0 first dimension')
                                 END IF
                                 ...
                                 IF (SIZE(CR0, 2) /= I0_tot) THEN
                                    ...
                                    CALL fail('Project_CRR0_2_CR0: inconsistent CR0 second dimension')
                                 END IF

                                 IF (SIZE(XP) < NNX) THEN
                                    WRITE (*, *) 'Project_CRR0_2_CR0: XP too small for NNX.'
                                    CALL fail('Project_CRR0_2_CR0: XP too small for NNX')
                                 END IF

                                 IF (SIZE(ZP) < NPT) THEN
                                    WRITE (*, *) 'Project_CRR0_2_CR0: ZP too small for NPT.'
                                    CALL fail('Project_CRR0_2_CR0: ZP too small for NPT')
                                 END IF

                                 IF (SIZE(CRR0_NPT, 1) < IANISO .OR. SIZE(CRR0_NPT, 2) < NPT) THEN
                                    WRITE (*, *) 'Project_CRR0_2_CR0: CRR0_NPT has inconsistent dimensions.'
                                    CALL fail('Project_CRR0_2_CR0: CRR0_NPT has inconsistent dimensions')
                                 END IF
   ...
   !-----------------------------------------------------------------------
   ! Legacy and alternative grid-remapping helpers
   !
   ! These routines support canonical-grid reconstruction and alternative
   ! block/NPT transfer pathways. They are kept together here because they
   ! are not part of the main frequency-band mesh-construction sequence.
   !-----------------------------------------------------------------------

   ! Build_Block_NodeDepths: Builds physical block-node depths from topography.
   ...
   ! NearestIndexInColumn: Finds the nearest canonical depth index within one column.
   ...
   ! Build_Block2Canon_Map: Maps block-column centers to canonical physical columns.
                              SUBROUTINE Build_Block2Canon_Map(X, NX, XM0, MX0, map_i2k)
                                 ...
                                 ! Build Voronoi boundaries from XM0 midpoints.
                                 ALLOCATE (edge(MX0 + 1))
                                 ...
                                 ! Map each block-column center X(i) to edge(k) <= X(i) < edge(k+1).
                                 DO i = 1, NX
                                    lo = 1
                                    hi = MX0
                                    DO WHILE (lo < hi)
   ...
   ! GRID_TRANSFORM_NPT2BLOCK: Interpolates an NPT field onto the block grid.
                              SUBROUTINE GRID_TRANSFORM_NPT2BLOCK(NNX, NNZ, NX, NZ, X, Z, FFF, NTO, XTO, ZTO, F)
                                 ...
                                 ! In forward code:
                                 !   DX = (XB-XA)/REAL(NNX-3,dp); inner i maps to fine ii=2..NNX-1.
                                 ...
                                 DO ii = 2, NNX - 1
                                    ...
                                    END DO
                                 END DO
                                 IF (nnint > 0) FAIR = FAIR/REAL(nnint, dp)
                                 ...
                                 DO I = 1, NX
                                    ...
                                    DO J = 1, NZ
                                       ...
                                       ! Above topography, retain the fallback average FAIR.
                                       IF (ZK > Z0) THEN
                                          F(ip) = FAIR
                                          CYCLE
                                       END IF
                                       ...
                                       xg = XA + REAL(ii0 - 2, dp)*DX
                                       zg = ZA + REAL(jj0 - 2, dp)*DZ
   ...
   ! GRID_TRANSFORM_NPT2BLOCK_ADJ: Applies the adjoint NPT-to-block transformation.
   ...
   ! map_block2npt_all: Maps all active block-parameter vectors to their NPT fields.
